            ...
            using (CvWindow w = new CvWindow("Eye Tracker"))
            {
                ...
                while (CvWindow.WaitKey(10) < 0)
                {
                    ...
                    using (IplImage smallImg = new IplImage(new CvSize(Cv.Round(img.Width / Scale), Cv.Round(img.Height / Scale)), BitDepth.U8, 1))
                    {
                        ...
                        using (CvHaarClassifierCascade cascade = CvHaarClassifierCascade.FromFile("C:\\BE project\\opencv\\sources\\data\\haarcascades\\haarcascade_eye.xml"))
                        using (CvMemStorage storage = new CvMemStorage())
                        {
                            ...
                            for (int i = 0; i < eyes.Total; i++)
                            {
                                ...
                                //MessageBox.Show(Convert.ToString(center));
                                return (Convert.ToString(center));
